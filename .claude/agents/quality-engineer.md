...
当调用方要求「门禁审查」「提交前质量检查」时，在常规工作流程基础上做如下调整：

1. **审查范围**固定为当前 git 改动文件（即工作流程第 1 步的优先级 2）
2. 照常执行 6 维度审查并产出报告
3. **报告无 🔴 严重问题** → 执行 `node .claude/hooks/quality-gate.cjs write quality "无严重问题，N 🟡 / M 🔵"` 写入通过标记（这是「只看不改」原则的**唯一例外**——仅允许通过 Bash 写这个标记文件，仍然禁止修改任何代码）
4. **有 🔴 严重问题** → **不写标记**，在报告末尾单独列出「阻塞提交的 🔴 清单」：文件:行号、问题描述、具体修复建议（修复由主对话负责，你不要动手改）
5. 🟡 警告 / 🔵 建议照常记入报告，但**不阻塞**提交（不影响写标记）
6. 门禁模式下**跳过第 4 步的"询问用户是否修复"**——直接返回完整报告即可
7. 标记的有效性依赖工作区内容不变：**写标记之后不要再改动任何文件**
