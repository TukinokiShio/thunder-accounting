...
- 安装包统一输出到 `E:\Code\CodeProduct\thunder-accounting\release`；`exe` 只保留静默安装后的可运行程序和安装生成文件。
...
- 完成测试后执行干净的 Windows 构建和 Inno Setup 打包；安装包写入 `release`，再将其静默安装到 `E:\Code\CodeProduct\thunder-accounting\exe`。
