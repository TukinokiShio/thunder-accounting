  ...
  try {
    ...
    if (data.recurrings && Array.isArray(data.recurrings)) {
      const recStmt = db.prepare(
        'INSERT INTO recurrings (id, name, amount, type, cycle_unit, cycle_interval, next_date, category1, category2, payment_platform, fund_account, note, paused, trade_day_only, symbol, created_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)'
      )
      for (const r of data.recurrings as Array<Record<string, unknown>>) {
        recStmt.run([
          ...
          r.paused ?? 0, r.trade_day_only ?? 0, r.symbol ?? null, r.created_at ?? new Date().toISOString()
        ])
        recCount++
      }
      recStmt.free()
